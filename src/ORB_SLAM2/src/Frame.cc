...
void Frame::AssignFeaturesToGrid()
{
    ...
    */
    ...
    for(int i=0;i<N;i++)
    {
        ...
        if(PosInGrid(kp,nGridPosX,nGridPosY))   // Compute the cell of a keypoint (return false if outside the grid)
...
void Frame::UpdatePoseMatrices()
{
    ...
    *   slam book P55
    */
    mOw = -mRcw.t()*mtcw;   // -RT*t   ///< mtwc,Translation from camera to world
}
 ...
 */
// 1. positive depth; 2. inside the image; 3. viewing angle < cos60;
bool Frame::isInFrustum(MapPoint *pMP, float viewingCosLimit)
{
    ...
    const float dist = cv::norm(PO);    // NORM_L2
    ...
    const float viewCos = PO.dot(Pn)/dist;  // cos=<P0,Pn>/|P0|*1
    ...
    pMP->mTrackProjX = u;
    pMP->mTrackProjXR = u - mbf*invz; //该3D点投影到双目右侧相机上的横坐标 z=fb/d; d=fb/z; d=ul-ur; ur=d-ul
    pMP->mTrackProjY = v;
...
void Frame::UndistortKeyPoints()
{
    ...
    */
    ...
    mat=mat.reshape(2);     // N*2 -> N*1*2(channel)
    cv::undistortPoints(mat,mat,mK,mDistCoef,cv::Mat(),mK); // 用cv的函数进行失真校正
    mat=mat.reshape(1);     // N*1*2(channel) -> N*2
...
void Frame::ComputeImageBounds(const cv::Mat &imLeft)
{
    if(mDistCoef.at<float>(0)!=0.0)
    {
        ...
        // Undistort corners
        mat=mat.reshape(2);     // 4*2 -> 4*1*2(channel).
        cv::undistortPoints(mat,mat,mK,mDistCoef,cv::Mat(),mK);     // mK: Calibration matrix
        mat=mat.reshape(1);     // 4*1*2(channel) -> 4*2
...
{
    // 对于双目，mvuRight存储了左目像素点在右目中的对应点的横坐标(u=col)
    ...
    vector<vector<size_t> > vRowIndices(nRows,vector<size_t>());    // init

    // vRowIndices[nRows][200]
    for(int i=0; i<nRows; i++)
        vRowIndices[i].reserve(200);    // reserve space

    // mvKeysRight:原始右图像提取出的特征点（未校正）
    const int Nr = mvKeysRight.size();
    // for every kp in right image
    for(int iR=0; iR<Nr; iR++)
    {
        // !!在这个函数中没有对双目进行校正，双目校正是在外层程序中实现的
        const cv::KeyPoint &kp = mvKeysRight[iR];                       //  ----> x
        const float &kpY = kp.pt.y;                                     //  |
        // 计算匹配搜索的纵向宽度，尺度越大（层数越高，距离越近），搜索范围越大    //  \/ y
        // 如果特征点在金字塔第一层，则搜索范围为:正负2
        // 尺度越大其位置不确定性越高，所以其搜索半径越大
        const float r = 2.0f*mvScaleFactors[mvKeysRight[iR].octave];
        const int maxr = ceil(kpY+r);   // ceil(x)返回的是大于x的最小整数。
        const int minr = floor(kpY-r);  // floor(x)返回的是小于或等于x的最大整数。

        for(int yi=minr;yi<=maxr;yi++)
            vRowIndices[yi].push_back(iR);  // 特征点编号
    }
    ...
    // mbf: Stereo baseline multiplied by fx; mbf=b*fx
    // ***********????????*********************
    const float maxD = mbf/minZ;  // 最大视差, 对应最小深度 mbf/minZ = mbf/mb = mbf/(mbf/fx) = fx (wubo???)

    // For each left keypoint search a match in the right image
    vector<pair<int, int> > vDistIdx;
    vDistIdx.reserve(N);    // N: Number of KeyPoints in left image.
    ...
    for(int iL=0; iL<N; iL++)
    {
        ...
        */
        ...
        // 可能的匹配点
        const vector<size_t> &vCandidates = vRowIndices[vL];    // vRowIndices[nRows][200]
        ...
        if(maxU<0)
            continue;
        // best match & match index
        int bestDist = ORBmatcher::TH_HIGH;
        size_t bestIdxR = 0;

        // 每个特征点描述子占一行，建立一个指针指向iL特征点对应的描述子
        // 左目摄像头和右目摄像头特征点对应的描述子 cv::Mat mDescriptors, mDescriptorsRight;
        ...
        for(size_t iC=0; iC<vCandidates.size(); iC++)
        {
            ...
            if(uR>=minU && uR<=maxU)
            {
                ...
                const int dist = ORBmatcher::DescriptorDistance(dL,dR); // haming dist
        ...
        if(bestDist<thOrbDist)
        {
            ...
            IL.convertTo(IL,CV_32F);            // ones : 单位矩阵
...
cv::Mat Frame::UnprojectStereo(const int &i)
{
    ...
    if(z>0)
    {
        ...
        cv::Mat x3Dc = (cv::Mat_<float>(3,1) << x, y, z);   // Pc
        return mRwc*x3Dc+mOw;   // Pw = Rwc*Pc + twc
    }
    else
        return cv::Mat();
}
